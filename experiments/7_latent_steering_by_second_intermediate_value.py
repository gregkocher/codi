# %%
# ABOUTME: Tests latent vector steering by patching with averaged vectors from specific second intermediate values.
# ABOUTME: Evaluates if patching a latent position can steer the model to produce a target second intermediate computation.
"""
Latent Steering by Second Intermediate Value Experiment

This script tests whether patching a specific latent vector position with an averaged
latent vector (computed from prompts with a specific second intermediate value (X+Y)*Z)
can steer the model's output to match that intermediate value's expected answer.

Approach:
1. For each template, mean-ablate the prompt activations
2. Compute averaged latent vectors grouped by second intermediate value (X+Y)*Z
3. For each test case:
   - Baseline: Mean-Abl Prompt + Patched Original Latents
   - Intervention: Patch one latent position with averaged vector for target second intermediate value
   - Check if output matches expected answer for the patched second intermediate value
"""
...
RESULTS_DIR = (
    Path(__file__).parent.parent / "results" / "latent_steering_second_intermediate"
)

# Range of second intermediate values to test ((X+Y)*Z ranges from 2 to 98 with X,Y,Z in 1-7)
# Using a smaller range for initial testing
MIN_SECOND_INTERMEDIATE = 4
MAX_SECOND_INTERMEDIATE = 5
...
def get_first_intermediate(X, Y):
    """
    Compute first intermediate value: X + Y
    """
    return X + Y


def get_second_intermediate(X, Y, Z):
    """
    Compute second intermediate value: (X+Y) * Z
    """
    return (X + Y) * Z
...
def get_answer(X, Y, Z):
    """
    Compute ground truth answer: (X+Y) * (1+Z) = (X+Y) + (X+Y)*Z
    """
    first_intermediate = X + Y
    second_intermediate = first_intermediate * Z
    answer = first_intermediate + second_intermediate
    return answer


def get_answer_from_second_intermediate(first_intermediate, second_intermediate):
    """
    Compute answer given first intermediate (X+Y) and second intermediate (X+Y)*Z.
    Answer = first_intermediate + second_intermediate
    """
    return first_intermediate + second_intermediate
...
def compute_mean_latent_embeddings_by_second_intermediate_value(
    model,
    tokenizer,
    ...
    """
    Compute mean latent embeddings grouped by second intermediate value (X+Y)*Z.

    For each second intermediate value in range MIN_SECOND_INTERMEDIATE to MAX_SECOND_INTERMEDIATE,
    computes mean latent vectors across all other templates with prompts
    that yield that second intermediate value.

    Returns:
        Dict mapping second_intermediate_value -> list of mean latent embeddings per step
    """
    num_templates = get_num_templates()
    all_templates = get_all_templates()

    # Group combinations by second intermediate value
    combos_by_second_intermediate = {}
    for X, Y, Z in all_combinations:
        second_intermediate = get_second_intermediate(X, Y, Z)
        if second_intermediate not in combos_by_second_intermediate:
            combos_by_second_intermediate[second_intermediate] = []
        combos_by_second_intermediate[second_intermediate].append((X, Y, Z))

    mean_embeddings_by_second_intermediate = {}

    for second_intermediate_value in range(
        MIN_SECOND_INTERMEDIATE, MAX_SECOND_INTERMEDIATE + 1
    ):
        if second_intermediate_value not in combos_by_second_intermediate:
            continue

        combos_for_second_intermediate = combos_by_second_intermediate[
            second_intermediate_value
        ][:]
        np.random.shuffle(combos_for_second_intermediate)
        all_latent_embeddings = []

        # Sample from combinations for this second intermediate value
        sampled_combos = combos_for_second_intermediate[
            :5
        ]  # Limit samples per second intermediate
        ...
        for template_idx in range(num_templates):
            ...
            for X, Y, Z in sampled_combos:
                ...
                print("Prompt", prompt)
                print("Second Intermediate", second_intermediate_value)
        ...
        mean_embeddings_by_second_intermediate[second_intermediate_value] = (
            mean_latent_embeddings
        )

    return mean_embeddings_by_second_intermediate
...
def main(
    ...
    """
    Run the latent steering experiment for second intermediate value.
    ...
    Args:
        ...
        patch_position: Which latent position to patch (0 to num_latents-1), default 1 for second step
    """
    load_dotenv()
    ...
    print(
        f"Second intermediate range: {MIN_SECOND_INTERMEDIATE} to {MAX_SECOND_INTERMEDIATE}"
    )
    ...
    for template_idx in range(num_templates):
        ...
        # Compute mean latent embeddings by second intermediate value
        print("Computing mean latent embeddings by second intermediate value...")
        mean_embeddings_by_second_intermediate = (
            compute_mean_latent_embeddings_by_second_intermediate_value(
                model,
                tokenizer,
                template_idx,
                all_combinations,
        ...
        print(
            f"  Computed for second intermediate values: {sorted(mean_embeddings_by_second_intermediate.keys())}"
        )
        ...
        for i, (X, Y, Z) in enumerate(test_case_combinations):
            ...
            first_intermediate = get_first_intermediate(X, Y)
            second_intermediate = get_second_intermediate(X, Y, Z)
        ...
        steering_results = {
            ...
            for iv in range(MIN_SECOND_INTERMEDIATE, MAX_SECOND_INTERMEDIATE + 1)
        }

        for test_case in tqdm(template_test_cases, desc=f"Template {template_idx + 1}"):
            prompt = test_case["prompt"]
            ground_truth = test_case["ground_truth"]
            first_intermediate = test_case["first_intermediate"]
            original_second_intermediate = test_case["second_intermediate"]
            print("Prompt", prompt)
            ...
            try:
                ...
                case_result = {
                    ...
                    "ground_truth": ground_truth,
                    "first_intermediate": first_intermediate,
                    "original_second_intermediate": original_second_intermediate,
                    "baseline_answer": baseline_answer,
                    "baseline_correct": baseline_correct,
                    "steering_results": {},
                }

                # Test steering for each target second intermediate value
                for (
                    target_second_intermediate
                ) in mean_embeddings_by_second_intermediate.keys():
                    if target_second_intermediate == original_second_intermediate:
                        # Skip if same as original
                        continue

                    target_latents = mean_embeddings_by_second_intermediate[
                        target_second_intermediate
                    ]
                    # Expected answer: first_intermediate + target_second_intermediate
                    target_answer = get_answer_from_second_intermediate(
                        first_intermediate, target_second_intermediate
                    )
                    ...
                    case_result["steering_results"][target_second_intermediate] = {
                        "target_answer": target_answer,
                        "steered_answer": steered_answer,
                        "correct": steered_correct,
                    }

                    steering_results[target_second_intermediate]["total"] += 1
                    if steered_correct:
                        steering_results[target_second_intermediate]["correct"] += 1
    ...
    ax1.set_title(
        "Latent Steering (2nd Intermediate): Overall Accuracy",
        fontsize=14,
        fontweight="bold",
        pad=10,
    )
    ...
    # Plot 2: Steering accuracy by second intermediate value
    ax2 = axes[1]
    iv_values = []
    iv_accs = []
    for iv in range(MIN_SECOND_INTERMEDIATE, MAX_SECOND_INTERMEDIATE + 1):
    ...
    ax2.plot(iv_values, iv_accs, "o-", color="#e74c3c", linewidth=2, markersize=6)
    ax2.set_xlabel(
        "Target Second Intermediate Value (X+Y)*Z", fontsize=12, fontweight="bold"
    )
    ...
    plt.tight_layout()
    chart_filename = RESULTS_DIR / "latent_steering_second_intermediate_results.png"
        ...
        json.dump(
            {
                "config": {
                    ...
                    "second_intermediate_range": [
                        MIN_SECOND_INTERMEDIATE,
                        MAX_SECOND_INTERMEDIATE,
                    ],
                },
                "overall_results": {
